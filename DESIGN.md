...
| 7 | **Concierge Agent** | Venue hours, public-vs-members info, membership enquiry, restaurant deflection detail | `Knowledge` = 1 | Public/no-auth. Answers ONLY from the Talkdesk **Knowledge** skill (Knowledge Management / RAG, indexed from the Ned London FAQ); never the model's world knowledge; offers a consent-gated human hand-off (`KNOWLEDGE_GAP`) when the FAQ has no answer. Keeps general questions out of transactional agents' prompts |
...
| Spa same-day notice | A treatment on `today` must be booked ≥ 2h before its slot. **Enforced in SQL (migration 14), not the model:** `get_activity_availability` computes `now() at time zone 'Europe/London'` and returns only today's slots ≥ `now + 2h` (past dates return nothing; future dates unaffected), and `post_activity_booking` re-checks the same 2h window as an authoritative write gate (a stale slot within it → `NO_AVAILABILITY`, which the Spa agent's re-check loop handles). The agent's Step 1.0 clock + STEP 4 same-day drop remain as a redundant secondary check. Moved SQL-side because the weak model did not apply the time compare reliably (offered already-passed slots), and availability/time determinism must live in Postgres per the rule-5 thesis | Spa |
