...
```
/supabase
  /migrations        -- numbered SQL migrations: 01_schema, 02_functions, 03_seed_static, 04_demo_functions, 05_harden, 06_entitlement_fields, 07_membership_id, 08_auth_events, 09_fix_upgrade_greeting, 10_cancel_activity_booking, 11_console_agents, 12_fix_reset_demo_delete, 13_guest_profile_crud, 14_spa_time_gate, 15_grant_membership, 16_prearrival_service_requests, 17_viewer_role
...
- **Backend:** Supabase (hosted). SQL functions are `SECURITY DEFINER`, return `json`. **Status: built, deployed, and tested** — **migrations `01`–`17` are ALL applied** to the provisioned project and pass the test checklist. (10 = `cancel_activity_booking`; 11 = `agents` staff table for console auth; 12 = `reset_demo` unqualified-DELETE fix — any function called over the service-role/PostgREST path must qualify every DELETE/UPDATE, because that connection enforces a "requires WHERE clause" guard the SQL editor/postgres role does not; 13 = `create/update/delete_guest_profile` for the console Customers tab, writes-via-SQL per rule 4, delete guarded against guests with history; 14 = spa same-day time gate moved into SQL — `get_activity_availability` + `post_activity_booking` compute `now() at time zone 'Europe/London'` and drop/reject today's slots earlier than `now + 2h`, because the weak model was not applying the agent-side Step-1 clock filter and offered already-passed slots; 15 = `grant_membership(profile_id[, enrollment_date])` — staff/agent "make someone a member" write-via-SQL, idempotent GRANTED/REACTIVATED/ALREADY_MEMBER/NOT_FOUND, allocates the next `M####`, London-dated enrollment, keeps `is_member`/`membership_years` computed; wired to the console Guest 360 "Grant membership" button; 16 = `prearrival_service_requests` — `post_service_request` now also accepts a pre-arrival guest (nearest upcoming `Reserved` stay → room `'On arrival'`, `pre_arrival:true`, `arrival_date`; `NOT_IN_HOUSE` only when there's no current or upcoming stay), adds `service_requests.pre_arrival`, and `get_service_requests` returns `pre_arrival`; Guest Services agent no longer escalates a not-in-house guest and words pre-arrival requests as held-for-arrival; console board flags them; 17 = `viewer_role` — adds a read-only `'viewer'` role to the `agents` role CHECK (was csr/supervisor/admin). Read-only is enforced in the Next.js backend, NOT SQL: new `requireWriter` guard in `console/lib/admin-guard.ts` 403s viewers and is used by every mutating API route (the GET/read routes keep `requireAuth`); the console UI also hides all write controls for viewers via `useSessionAgent().canWrite`. Demo read-only logins `alice`/`bob`/`carol`@thewren-demo.com (password `DemoPass123!`) are seeded idempotently by `console/scripts/seed-demo-agents.ts` (`npm run seed:demo-agents`).) Apply migrations with the Supabase CLI (`supabase db push`) or the Supabase MCP `apply_migration`. The project ref + service-role key live in `console/.env.local` (gitignored) and project memory — never commit them.
...
- Because access is service-role via the backend, enable RLS on all tables with no anon policies (belt-and-braces: even a leaked anon key reads nothing). **Console auth (UPDATED, user-directed):** the console v1 now DOES have staff authentication (login + roles csr/supervisor/admin/viewer + change-password), ported from the crestline partner-core pattern — this supersedes the earlier "no console auth in v1" rule. `viewer` is read-only (migration 17): blocked from every write by the `requireWriter` guard, and its write UI is hidden via `useSessionAgent().canWrite`. Staff accounts live in the `agents` table (migration 11); sessions are bcrypt + `jose` JWT httpOnly cookies; secrets (`JWT_SECRET`, `SUPABASE_SERVICE_ROLE_KEY`) are server-only in `console/.env.local`.
...
- `post_service_request('P1002','EXTRA_BLANKET',1,null)` → in-house row with room '412', department 'Housekeeping', eta_text 'within 30 minutes', `pre_arrival` false
- `post_service_request('P1001','EXTRA_TOWELS',2,null)` → **pre-arrival** row (Thompson not checked in, upcoming stay): room 'On arrival', `pre_arrival` true, `arrival_date` set, department 'Housekeeping' (migration 16)
- `post_service_request(<profile with no current or upcoming stay>, …)` → NOT_IN_HOUSE (fires only when the guest has neither a CheckedIn nor an upcoming Reserved stay)
