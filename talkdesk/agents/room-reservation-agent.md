...
**Character count:** 10,342 (measured; limit 20,000). Re-measure with `printf '%s' | wc -c` after any edit.
...
- Measured instruction count: 10,342 characters (limit 20,000). Re-measure after any edit.
...
- Currency is written as the word "pounds" (e.g. "450 pounds a night"), never the `£` glyph: GPT-4.1-mini can't reliably reproduce non-ASCII characters and emitted "A3" (the code point of `£`), turning "£450" into "A3450" in a live confirmation email. Keep all templates ASCII. If a real `£` glyph is ever wanted in the HTML email, use the entity `&pound;` in `body_html` only.
