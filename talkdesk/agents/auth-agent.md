...
**Character count:** 7,830 (measured; limit 20,000). Re-measure with `printf '%s' | wc -c` after any edit.
...
STEP 3 — VERIFY
If Channel Name is WHATSAPP AND the intent is Tier 1: no code is needed; the channel's verified number stands as identification. Go to Step 4.
...
Via execute_sql call get_entitlement_context for the profile; it returns the guest's profile, membership, and stay data. Then call set_customer_context, writing exactly these 13 variables. Pass straight through: profile_id, name_given, name_surname, email, phone, in_house_room. Flatten upcoming_stay into three strings — upcoming_arrival_date, upcoming_departure_date, and upcoming_confirmation_number — each set from the matching field of upcoming_stay, or an empty string if there is no upcoming stay. Write the true/false values is_member, in_house, phone_identified, and authenticated as the strings "true" or "false".
Set the flags by how identity was established: phone_identified = "true" whenever you have a trusted number — a WhatsApp verified sender, or a number confirmed by a passing OTP, otherwise "false"; authenticated = "true" only when Step 3's OTP returned MATCH, otherwise "false". For WHATSAPP + Tier 1, phone_identified = "true" and authenticated = "false".
...
- Measured instruction count: 7,830 characters (limit 20,000). Re-measure after any edit.
